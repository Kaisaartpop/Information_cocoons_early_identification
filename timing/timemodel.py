...
i=args.seq_len*5
data_temporal_train= np.load(f'../data/data_full_{i}/X_train_temporal.npy')
data_static_train=np.load(f'../data/data_full_{i}/X_train_static.npy')
labels_train=np.load(f'../data/data_full/y_train.npy')
data_temporal_val= np.load(f'../data/data_full_{i}/X_val_temporal.npy')
data_static_val=np.load(f'../data/data_full_{i}/X_val_static.npy')
labels_val=np.load(f'../data/data_full/y_val.npy')
data_temporal_test= np.load(f'../data/data_full_{i}/X_test_temporal.npy')
data_static_test=np.load(f'../data/data_full_{i}/X_test_static.npy')
labels_test=np.load(f'../data/data_full/y_test.npy')
# Step 3: Define PyTorch Dataset class
print(data_temporal_train.shape)
class CustomDataset(Dataset):




    def __init__(self, data_temporal, data_static, labels):
    ...
    def __getitem__(self, idx):
        ...
        x_static = self.data_static[idx]
        y = self.labels[idx]
        return x_static, x_temporal, y

# Create dataset objects
train_dataset = CustomDataset(data_temporal_train, data_static_train, labels_train)
val_dataset = CustomDataset(data_temporal_val, data_static_val, labels_val)
test_dataset = CustomDataset(data_temporal_test, data_static_test, labels_test)
# Create data loaders
batch_size =2048
...
class Cocoons_prediction(nn.Module):
    ...
    def forward(self, x, x_temporal):
        x, x_temporal = x.to(device), x_temporal.to(device)
        x_temporal[:,8:8+35,:]/=18000
        N = x_temporal.shape[0]
        x_temporal=torch.cat([x_temporal[:,:1,:],x_temporal[:,8:43,:]],dim=1)
       
        
        channel_outputs = [channel(x_temporal[:, i, :].unsqueeze(1)) for i, channel in enumerate(self.channels)]
        #xc = torch.stack(channel_outputs, dim=1)  # (N, variable, 64*3)
        channel_outputs = torch.stack(channel_outputs, dim=1) 
        
        cat_outputs = [channel(channel_outputs[:, i+1, :]) for i, channel in enumerate(self.addict)]
  ...
  # (batch_size, node, embed_dim)
        K = torch.einsum('bni,nii->bni', cat_outputs, self.W_k)+self.b_k.unsqueeze(0).unsqueeze(-1) # (batch_size, node, embed_dim)
        Q_norm = torch.norm(Q, p=2, dim=-1, keepdim=True)  # (batch_size, node, 1)
        K_norm = torch.norm(K, p=2, dim=-1, keepdim=True)  # (batch_size, node, 1)

      
        # cosine_similarity = torch.matmul(Q, K.transpose(-2, -1)) / (Q_norm * K_norm.transpose(-2, -1)+ 1e-8)  # (batch_size, node, node)
        cosine_similarity = (torch.matmul(Q, K.transpose(-2, -1))) / (Q_norm * K_norm.transpose(-2, -1) + 1e-8)  # (batch_size, node, node)
        ...
        cat_outputs=torch.matmul(torch.eye(35).to(device)+F.softmax(F.relu(self.adj), dim=1),cat_outputs)
        total_outputs=torch.cat([channel_outputs[:,1:,:],cat_outputs1,cat_outputs],dim=2)
        decode=[channel(total_outputs[:, i, :]) for i, channel in enumerate(self.decoder)]
        decode=torch.cat(decode,dim=1)
         
 
        xc = self.fc(torch.cat([channel_outputs[:,0,:].reshape(N,-1),x_temporal.reshape(N,-1),decode.reshape(N,-1)],dim=1))
...
for epoch in range(num_epochs):
    ...
    for x_static, x_temporal, y in train_loader:
        y=y.to(device)
        optimizer.zero_grad()
        outputs = model(x_static, x_temporal)
    ...
    with torch.no_grad():
        print('val')
        for x_static, x_temporal, y in val_loader:
            y=y.to(device)
            outputs = model(x_static, x_temporal)
            loss = criterion(outputs.squeeze(), y)
            val_loss += loss.item()
...
with torch.no_grad():
    print('val')
    for x_static, x_temporal, y in test_loader:
        y=y.to(device)
        outputs = model(x_static, x_temporal)
        loss = criterion(outputs.squeeze(), y)
        val_loss += loss.item()
        ...
        true_labels.extend(y.cpu().numpy())
predictions = np.array(predictions)
np.save(f'org_full_{i}.npy',predictions)
predicted_labels = (predictions > thrs).astype(int)
y_test=np.array(true_labels)
...
with torch.no_grad():
    ...
    for x_static, x_temporal, y in train_loader:
        ...
        true_labels.extend(y.cpu().numpy())
predictions = np.array(predictions)
np.save('m2_att2_train_norm.npy',predictions)
predicted_labels = (predictions > thrs).astype(int)
y_test=np.array(true_labels)
